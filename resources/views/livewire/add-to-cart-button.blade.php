<div>
    @if($variant === 'text')
        <button wire:click="addToCart"
            ...
            @else
                ...
                <span class="text-xs sm:text-sm">Masukkan Keranjang</span>
            @endif
        </button>
    @elseif($variant === 'buy')
        <button wire:click="buyNow"
            class="w-full h-12 lg:h-14 bg-primary hover:bg-neutral-dark text-white rounded-[1.25rem] flex items-center justify-center gap-2 text-xs sm:text-base font-black transition-all shadow-2xl shadow-primary/30 hover:-translate-y-1 relative overflow-hidden">
            <!-- Loading State -->
    ...
    @else
        <button wire:click="addToCart"
            class="w-7 h-7 sm:w-10 sm:h-10 bg-primary text-white rounded-lg flex items-center justify-center transition-all duration-300 shadow-lg shadow-primary/20 hover:bg-primary-dark hover:scale-110 active:scale-95 group/btn relative overflow-hidden">
            <!-- Loading State -->
            <div wire:loading wire:target="addToCart"
                class="absolute inset-0 bg-primary flex items-center justify-center z-10">
                <svg class="w-4 h-4 animate-spin text-white" fill="none" viewBox="0 0 24 24 text-white">
                    <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
                    <path class="opacity-75" fill="currentColor"
                        d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z">
                    </path>
            ...
            @if($isAdded)
                <svg class="w-4 h-4 md:w-5 md:h-5 animate-ping" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M5 13l4 4L19 7"></path>
                </svg>
            @else
                <svg class="w-4 h-4 md:w-5 md:h-5 transition-transform group-hover/btn:scale-110" fill="none"
