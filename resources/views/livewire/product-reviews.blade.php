    ...
    @if($totalReviews > count($reviews))
        <div class="flex justify-center pt-2">
            <button wire:click="loadMore" wire:loading.attr="disabled"
                class="flex items-center gap-2 px-6 py-2.5 bg-white border border-slate-200 rounded-xl font-bold text-xs text-neutral-dark hover:bg-slate-50 transition-all shadow-sm active:scale-95 group">
                ...
                <svg wire:loading.remove wire:target="loadMore"
                    class="w-4 h-4 text-primary transition-transform group-hover:translate-y-0.5" fill="none"
