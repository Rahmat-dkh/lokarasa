            ...
            <div class="bg-white rounded-[2.5rem] shadow-sm border border-slate-100 overflow-hidden">
                @if($orders->isNotEmpty())
                    ...
                    <div class="sm:hidden divide-y divide-slate-50">
                        @foreach($orders as $order)
                            <a href="{{ route('vendor.orders.show', $order) }}" class="block p-5 hover:bg-slate-50 transition-colors">
                                <div class="flex justify-between items-start mb-3">
                                    <div class="flex flex-col">
                                        <span class="text-[10px] font-black text-primary uppercase tracking-widest mb-1">ID #{{ str_pad($order->id, 5, '0', STR_PAD_LEFT) }}</span>
                                ...
                                <div class="flex justify-between items-center mt-4">
                                    <div class="flex flex-col">
                                        <span class="text-[10px] font-bold text-slate-700">{{ $order->created_at->format('d M Y') }}</span>
                                        <span class="text-[9px] font-black text-slate-400 uppercase tracking-widest">{{ $order->created_at->format('H:i') }} WIB</span>
                                    </div>
                                    <span class="text-sm font-black text-primary italic">Rp {{ number_format($order->total_amount - $order->service_fee, 0, ',', '.') }}</span>
                                </div>
                            </a>
